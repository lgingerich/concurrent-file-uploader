...
async def async_upload(storage_client: storage.Client, bucket_name: str, data_dir: str, csv_files: List[str]) -> None:
    ...
    for csv_file in csv_files:
        ...
        if os.path.exists(source_file_path):
            await upload_blob(storage_client, bucket_name, source_file_path)
        else:
            print(f"File not found: {source_file_path}")


async def upload_blob(storage_client: storage.Client, bucket_name: str, source_file_path: str) -> None:
    """
